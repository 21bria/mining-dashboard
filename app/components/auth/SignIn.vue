...
const handleLogin = async (): Promise<void> => {
  ...
  try {
    ...
    const accessToken = useCookie('access', { maxAge: 60 * 60 * 24 })
    const refreshToken = useCookie('refresh', { maxAge: 60 * 60 * 24 * 7 })
    const user = useCookie<{ name: string; email: string }>('user', { maxAge: 60 * 60 * 24 * 7 })
    ...
    console.log('[LOGIN] Token dan user berhasil disimpan.')
    ...
    if (e.response?.data) {
      ...
      error.value = errData[firstKey][0] || 'Login gagal.'
    } else {
...
<template>
  <form class="grid gap-6" @submit="onSubmit">
    ...
    <Button type="submit" class="w-full" :disabled="isLoading">
      <Loader2 v-if="isLoading" class="mr-2 h-4 w-4 animate-spin" />Login
    </Button>
  </form>
