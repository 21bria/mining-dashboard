...
const dummyData = Array.from({ length: 30 }, (_, i) => {
  ...
  const limonite = Math.floor(Math.random() * 500) + 100
  const plan = limonite + Math.floor(Math.random() * 100) - 180
...
//     {
//       name: 'Limonite',
//       data: dummyData.map(item => item.limonite)
//     },
//     {
//       name: 'Saprolite',
...
async function fetchChartData() {
  try {
    ...
    const url = `${URL_GET_DATA}dashboard/api/ore-chart/?${params.toString()}`
