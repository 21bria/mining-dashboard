...
const getThemeOptions = (): ApexOptions => ({
      ...
      function renderBarges(barges: (string | Barge)[] | undefined): string {
        ...
        ${barges.map((b: string | Barge) => {
          if (typeof b === 'object' && b !== null) {
            ...
            return `<li>
              <strong>${b.barge_code ?? 'Unknown'}</strong> - Lim: ${b.lim}, Sap: ${b.sap}, Total: ${b.total}
              ${nestedBarges}
            </li>`
          }
          return `<li>${b}</li>`
  ...
  fill: {
    ...
    type: ["gradient", "gradient", "solid"], // 2 barge pakai gradient, Plan pakai solid
    gradient: {
      inverseColors: false,
      shade: 'light',
      type: "vertical",
  ...
  stroke: {
    width: [0, 0, 0, 2, 3], // 2 bar = 0, sisanya line lebih tipis
    dashArray: [0, 0, 0, 4, 0] // plan LIM & SAP pakai garis putus-putus
  },
  markers: {
    size: [0, 0, 4], // hanya series terakhir (Plan) yg ada markernya
