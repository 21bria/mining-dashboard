<template>
  <div class="w-full flex flex-col gap-4">
    ...
    <div class="mb-2 flex items-center justify-between">
      <div>
        <h1 class="text-xl font-bold text-gray-900 dark:text-white">Inventory by Stockpile</h1>
      ...
      <CardContent>
        ...
        <div v-else>
          <Table>
            ...
            <TableBody>
              <TableRow v-for="(dome, i) in domes" :key="dome.stockpile"
                ...
                <TableCell class="text-left px-1 py-1.4">{{ dome.stockpile }}</TableCell>
                ...
                <TableCell class="text-right px-1 py-0.6">{{ format2(dome.ni) }}</TableCell>
                <TableCell class="text-right px-1 py-0.6">{{ format2(dome.fe) }}</TableCell>
                <TableCell class="text-right px-1 py-0.6">{{ format2(dome.al2o3) }}</TableCell>
                <TableCell class="text-right px-1 py-0.6">{{ format2(dome.co) }}</TableCell>
                <TableCell class="text-right px-1 py-0.6">{{ format2(dome.mgo) }}</TableCell>
                <TableCell class="text-right px-1 py-0.6">{{ format2(dome.sio2) }}</TableCell>
                <TableCell class="text-right px-1 py-0.6">{{ format2(dome.cao) }}</TableCell>
                <TableCell class="text-right px-1 py-0.6">{{ format2(dome.cr2o3) }}</TableCell>
                <TableCell class="text-right px-1 py-0.6">{{ format2(dome.sm) }}</TableCell>
              </TableRow>
              <!-- Summary row -->
...
async function fetchInventory() {
  ...
  try {
    ...
    const url = `${URL_GET_DATA}dashboard/api/inventory-stockpile/?${params.toString()}`
