<template>
  <div class="w-full flex flex-col gap-4">
    <!-- Header -->
    <div class="mb-0ß flex items-center justify-between">
      <div>
        <h1 class="text-xl font-bold text-gray-900 dark:text-white">Detail Inventory</h1>
      ...
      <CardContent>
        ...
        <div v-else>
          ...
          <!-- <tr ref="loadMoreTrigger">
            <td colspan="15" class="text-center py-4 text-sm text-gray-400">
              <span v-if="isLoading">Loading more...</span>
              <span v-else-if="!hasMore">No more data</span>
            </td>
          </tr> -->
...
async function fetchInventory() {
  ...
  try {
    ...
    const url = `${URL_GET_DATA}dashboard/api/inventory-data/?${params.toString()}`
    ...
    if (page.value === 1) {
      ...
      domes.value.push(...rows)
    }

    summary.value = data.summary || null

    // tentukan masih ada data atau tidak
...
// Filter berubah → reset
watch([saleFilter, areaFilter, pointFilter], () => {
  page.value = 1
  domes.value = []
  hasMore.value = true
  fetchInventory()
...
// Page berubah → load more
watch(page, (val) => {
  if (val > 1) fetchInventory()
})
